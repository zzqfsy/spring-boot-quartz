var TaskInfo = {
	...
	selectParam: function () {
		var myOptValue = $("#JobGroup").combobox("getValue");
		if (myOptValue == "HTTP_RPC_Group"){
			$("#ipAddress").hidden = false;
			$("#method").hidden = false;
			$("#param").hidden = false;
		}else {
			$("#ipAddress").hidden = true;
			$("#method").hidden = true;
			$("#param").hidden = true;
		}

		$('#taskinfoDialog').reload();
	},
	...
	deleteTaskInfo: function(){
		...
		Ext.confirm('您确认要删除这条记录吗?', function(){
			...
			var record = Ext.getRecord(TaskInfo.grid);
			$.post("/delete/", record, function(result){
	...
	pauseTaskInfo: function(index) {
		...
		Ext.confirm('您确认要暂停该任务吗?', function(){
			Ext.progress('正在暂停任务...');
			var record = Ext.getRecord(TaskInfo.grid);
			$.post("/pause/", record, function(result){
				if(result.errorCode==0){
					$(TaskInfo.grid).datagrid("reload")
				}else{
					Ext.alert(result.errorText);
	...
	resumeTaskInfo: function(index) {
		...
		Ext.confirm('您确认要重新开始该任务吗?', function(){
			Ext.progress('正在开始任务...');
			var record = Ext.getRecord(TaskInfo.grid);
			$.post("/resume/", record, function(result){
				if(result.errorCode==0){
					$(TaskInfo.grid).datagrid("reload")
				}else{
					Ext.alert(result.errorText);
	...
	initGrid: function(){
		$(this.grid).datagrid({
		    ...
		    columns:[[
				...
				{field:'jobClassName',title:'JobClassName',width:50},
			...
			onExpandRow: function(index,row){
				$('#ddv-'+index).panel({
					border:false,
					cache:false,
					href:'detail?jobName='+row.jobName+'&jobGroup='+row.jobGroup,
					onLoad:function(){
						$(this.grid).datagrid('fixDetailRowHeight',index);
					}
				});
				$(this.grid).datagrid('fixDetailRowHeight',index);
			}
	...
	createTaskInfoDialog: function() {
		...
		$('#jobClassName').combobox('select', "com.zzqfsy.task.job.HttpRPCJob");
